...
function updatePricingCardsColors(file) {
  ...
  // Let's find the map function start
  const mapStartStr = "            ].map((plan, i) => {";
  const startIndex = content.indexOf(mapStartStr);
  
  if (startIndex === -1) {
    console.error(`Could not find map start in ${file}`);
    return;
  }
  
  const endIndex = content.indexOf('          </div>', startIndex);
  if (endIndex === -1) {
    console.error(`Could not find end of map in ${file}`);
    return;
  }

  const newCardsCode = `            ].map((plan, i) => {
