...
function updateHome() {
  ...
  if (!content.includes('const [seats, setSeats]')) {
    ...
    // Replace existing if it was wrong
    content = content.replace(
  ...
  // Find the pricing grid to replace
  // In Home.jsx, SECTION 7 starts with <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? 'repeat(3, 1fr)' : '1fr', gap: 32, alignItems: 'stretch' }}>
  const gridMatch = content.match(/<div style=\{\{\s*display: 'grid',\s*gridTemplateColumns: isDesktop \? 'repeat\([34], 1fr\)' : '1fr',\s*gap: \d+,\s*alignItems: 'stretch'\s*\}\}>/);
  ...
  const startIndex = gridMatch.index;
