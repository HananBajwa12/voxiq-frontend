...
// Add seats state to Home.jsx
if (!homeContent.includes('const [seats, setSeats]')) {
...
const matchStart = /<div style=\{\{ display: 'grid', gridTemplateColumns: isDesktop \? 'repeat\(4, 1fr\)' : '1fr', gap: 24, alignItems: 'stretch' \}\}>/;
const startMatch = homeContent.match(matchStart);

if (!startMatch) {
  console.error("Could not find cards grid start in Home.jsx");
  process.exit(1);
}

const startIndex = startMatch.index;

const matchEnd = /<\/div>\s*<\/div>\s*<\/section>\s*\{\/\* ==/;
const endMatch = homeContent.match(matchEnd);

if (!endMatch) {
  console.error("Could not find cards grid end in Home.jsx");
  process.exit(1);
}

const endIndex = endMatch.index;

const newCardsCode = `          <div style={{ display: 'grid', gridTemplateColumns: isDesktop ? 'repeat(4, 1fr)' : '1fr', gap: 24, alignItems: 'stretch' }}>
            {[
              {
                id: 'basic',
                name: 'Basic',
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', marginBottom: '6px', minHeight: '44px' }}>
                        {isCustom ? (
                          ...
                          <>
                            <span style={{ fontSize: '2.8rem', fontWeight: 700, color: isPopular ? '#FFFFFF' : '#0A2540', letterSpacing: '-0.02em', fontFamily: "'Space Grotesk', sans-serif" }}>{displayPriceTop}</span>
                      ...
                      {!isCustom ? (
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: isPopular ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.02)', padding: '6px 12px', borderRadius: '8px', marginBottom: '24px' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: isPopular ? 'rgba(255,255,255,0.1)' : '#FFFFFF', padding: '4px 8px', borderRadius: '6px', border: isPopular ? '1px solid rgba(255,255,255,0.2)' : '1px solid #E2E8F0' }}>
                            <span style={{ fontSize: '1rem', cursor: 'pointer', color: isPopular ? '#FFFFFF' : '#0A2540', padding: '0 4px', userSelect: 'none' }} onClick={() => seats > 1 && setSeats(seats - 1)}>-</span>
                            <span style={{ fontSize: '0.9rem', fontWeight: 600, color: isPopular ? '#FFFFFF' : '#0A2540', minWidth: '16px', textAlign: 'center' }}>{seats}</span>
                            <span style={{ fontSize: '1rem', cursor: 'pointer', color: isPopular ? '#FFFFFF' : '#0A2540', padding: '0 4px', userSelect: 'none' }} onClick={() => setSeats(seats + 1)}>+</span>
                          </div>
                        </div>
                      ) : (
...
homeContent = homeContent.substring(0, startIndex) + newCardsCode + homeContent.substring(endIndex + 37); // +37 because `</div>\n        </div>\n      </section>` length is ~37, but we are providing our own `        </div>\n      </section>\n` in newCardsCode. Wait, let me just provide it exactly.
