...
// The original Pricing.jsx has a hardcoded grid for Starter, Growth, Enterprise.
// We need to find the start of the grid.
const gridStartMatch = content.match(/<div style=\{\{\s*display: 'grid',\s*gridTemplateColumns: 'repeat\(3, 1fr\)',\s*gap: '32px',\s*alignItems: 'stretch'\s*\}\} className="pricing-grid">/);

if (!gridStartMatch) {
  console.error("Could not find grid start in Pricing.jsx");
  process.exit(1);
}

const startIndex = gridStartMatch.index;
// Find the end of the pricing section container.
// In Pricing.jsx, the grid is followed by "</div>\n        </div>\n      </section>\n\n      {/* SECTION 3 — FEATURES TABLE */}"
const endIndex = content.indexOf("</section>", startIndex);

if (endIndex === -1) {
  console.error("Could not find grid end in Pricing.jsx");
  process.exit(1);
}

// Add independent seats state if it's missing (it was added by update_both but let's be sure)
...
} else if (content.includes('const [seats, setSeats] = useState(1);')) {
  content = content.replace(
...
// Ensure isAnnual is defined
if (!content.includes('const isAnnual = billingCycle === \'annual\';')) {
  content = content.replace(
    /const isTransitioning = false;/, // Just a fallback anchor if needed
    "const isTransitioning = false;\n  const isAnnual = billingCycle === 'annual';"
  );
}

const newCardsCode = `<div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(4, 1fr)', gap: '24px', alignItems: 'stretch' }} className="pricing-grid">
            {[
              {
                id: 'basic',
                name: 'Basic',
                ...
                >
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                      ...
                      <h3 style={{ fontSize: '1.35rem', fontWeight: 600, color: '#F1F5F9', margin: 0, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{plan.name}</h3>
                    </div>
                    ...
                    <div style={{ 
                      ...
                      {isCustom ? (
                        <span style={{ fontSize: '2.2rem', fontWeight: 700, color: '#F1F5F9', letterSpacing: '-0.02em', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                          Talk to Sales
                        </span>
                      ) : (
                        <>
                          <span style={{ fontSize: '2.8rem', fontWeight: 700, color: '#F1F5F9', letterSpacing: '-0.02em', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                            {displayPriceTop}
                          </span>
                    ...
                    {!isCustom ? (
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: isPopular ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.02)', padding: '6px 12px', borderRadius: '8px', marginBottom: '24px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: isPopular ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.05)', padding: '4px 8px', borderRadius: '6px', border: isPopular ? '1px solid rgba(255,255,255,0.2)' : '1px solid rgba(255,255,255,0.1)' }}>
                          <span style={{ fontSize: '1rem', cursor: 'pointer', color: '#F1F5F9', padding: '0 4px', userSelect: 'none' }} onClick={() => planSeats > 1 && setSeats(prev => ({ ...prev, [plan.id]: prev[plan.id] - 1 }))}>-</span>
                          <span style={{ fontSize: '0.9rem', fontWeight: 600, color: '#F1F5F9', minWidth: '16px', textAlign: 'center' }}>{planSeats}</span>
                          <span style={{ fontSize: '1rem', cursor: 'pointer', color: '#F1F5F9', padding: '0 4px', userSelect: 'none' }} onClick={() => setSeats(prev => ({ ...prev, [plan.id]: prev[plan.id] + 1 }))}>+</span>
                        </div>
                      </div>
                    ) : (
...
console.log('Updated Pricing.jsx successfully');
